...
# ---------------------------------------------------------
# 1. TAM KAPSAMLI VERİ HAZIRLIĞI (Makro Bağlam Dahil)
# ---------------------------------------------------------
def tam_kapsamli_veri_hazirligi():
    ...
    # Makro-Bağlam (Grup İstatistikleri)
...
# ---------------------------------------------------------
# 2. DEEP SEED BLENDING STACKING MİMARİSİ
# ---------------------------------------------------------
# Zırh gibi sağlam olması için 5 farklı tohum kullanıyoruz
seed_list = [42, 2026, 777, 1024, 888]
final_test_predictions = np.zeros(len(X_test))
seed_skorlari = []

print(f"\n🚀 GRANDMASTER MİMARİSİ BAŞLIYOR ({len(seed_list)} Tohum, 3 İşçi, 1 Yönetici)...")
print("Bu işlem 75 model eğiteceği için birkaç dakika sürebilir. Lütfen bekleyin.\n")
...
for i, seed in enumerate(seed_list):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y_binned)):
        ...
        # 2. ALTIN İŞÇİ: LIGHTGBM (Optuna Çıktısı)
        model_lgb = lgb.LGBMRegressor(
            n_estimators=1332, learning_rate=0.01078, max_depth=5, num_leaves=36, 
            subsample=0.6176, colsample_bytree=0.7123, random_state=seed, n_jobs=-1, verbose=-1
        )
        ...
        # 3. ALTIN İŞÇİ: XGBOOST (Optuna Çıktısı)
        model_xgb = xgb.XGBRegressor(
            n_estimators=1156, learning_rate=0.0129, max_depth=4, subsample=0.6543, 
            colsample_bytree=0.8145, enable_categorical=True, tree_method='hist', random_state=seed, n_jobs=-1
        )
...
print(f"\n=======================================================")
print(f"🌟 GRANDMASTER NİHAİ CV RMSE (5 Tohum Ortalaması): {np.mean(seed_skorlari):.5f} 🌟")
print(f"=======================================================")

# Dosyayı Kaydetme
# Dosyayı Kaydetme
submission_stacking = pd.DataFrame({'id': test_id, 'bilissel_performans_skoru': final_test_predictions})
dosya_adi = 'submission_GRANDMASTER_Deep_Stacking.csv'
submission_stacking.to_csv(dosya_adi, index=False)
print(f"Özel Zırhlı Dosyanız Hazır: {dosya_adi}")