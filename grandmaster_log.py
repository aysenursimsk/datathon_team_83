...
def tam_kapsamli_veri_hazirligi():
    ...
    for df in [train_df, test_df]:
        ...
        for col in sayisal_kolonlar:
            if col in df.columns and col != 'bilissel_performans_skoru': df[col] = df[col].fillna(df[col].median())
...
X, y_orijinal, X_test, test_id, kategorik_kolonlar = tam_kapsamli_veri_hazirligi()

# KAGGLE HİLESİ: HEDEF DÖNÜŞÜMÜ (LOG-TRANSFORM)
y_log = np.log1p(y_orijinal)
y_binned = pd.qcut(y_orijinal, q=10, labels=False, duplicates='drop')

# ---------------------------------------------------------
# 2. DEEP SEED BLENDING STACKING MİMARİSİ (LOG DÜZLEMİNDE)
# ---------------------------------------------------------
seed_list = [42, 2026, 777, 1024, 888]
final_test_predictions_log = np.zeros(len(X_test))
seed_skorlari = []

print(f"\n🚀 LOG-TRANSFORM GRANDMASTER MİMARİSİ BAŞLIYOR...")
...
for i, seed in enumerate(seed_list):
    ...
    # Log düzlemindeki tahminleri tekrar normal düzleme alıyoruz (expm1)
    nihai_train_tahminleri = np.expm1(meta_model.predict(X_meta_train))
    nihai_test_tahminleri_log = meta_model.predict(X_meta_test)
    
    # KAGGLE HİLESİ: ACIMASIZ TIRAŞLAMA (0-10 SINIRI)
    nihai_train_tahminleri = np.clip(nihai_train_tahminleri, 0.0, 10.0)
    
    seed_rmse = root_mean_squared_error(y_orijinal, nihai_train_tahminleri)
    print(f"[{i+1}/{len(seed_list)}] Tohum: {seed} | Ridge CV RMSE: {seed_rmse:.5f}")
    
    seed_skorlari.append(seed_rmse)
    final_test_predictions_log += nihai_test_tahminleri_log / len(seed_list)

print(f"\n=======================================================")
print(f"🌟 LOG-TRANSFORM NİHAİ CV RMSE: {np.mean(seed_skorlari):.5f} 🌟")
print(f"=======================================================")

# Test sonuçlarını normale çevirip tıraşlıyoruz
final_test_predictions = np.expm1(final_test_predictions_log)
final_test_predictions = np.clip(final_test_predictions, 0.0, 10.0)

# Dosyayı Kaydetme
submission_stacking = pd.DataFrame({'id': test_id, 'bilissel_performans_skoru': final_test_predictions})
dosya_adi = 'submission_LOG_TRANSFORM_Stacking.csv'
submission_stacking.to_csv(dosya_adi, index=False)
print(f"0-10 Tıraşlı Dosyanız Hazır: {dosya_adi}")